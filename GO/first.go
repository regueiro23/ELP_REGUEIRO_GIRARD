...
func main() {
	...
	for _, image := range photosComparees {
		...
		go func(img string) {
			...
			visagesCompares := sampleMultiplesVisages(rec, img)
			...
			if visagesCompares == nil {
				...
				for _, visage := range visagesCompares {
					...
					if IDVisage < 0 {
						...
						go enregistreCopieRectangle(img, visage.Rectangle.Min.X, visage.Rectangle.Min.Y, visage.Rectangle.Max.X, visage.Rectangle.Max.Y, labels[IDVisage], img)
					}
				}
			}
		}(image)
	...
	fmt.Println("Fin de l'analyse et de la comparaison")
